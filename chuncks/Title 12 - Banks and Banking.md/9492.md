...
(2)A Board-regulated institution must determine the exposure amount for an eligible margin loan, repo-style transaction, collateralized derivative contract, or a single-product netting set of such transactions by setting the exposure amount equal to max {0, [(ΣE - ΣC) + Σ(Es x Hs) + Σ(Efx x Hfx)]}, where:
