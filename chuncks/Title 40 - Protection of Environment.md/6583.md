...
The measurement uncertainties are estimated for each captured or uncaptured emissions point as follows: Q=+/-5.5 percent and C= +/-5 percent. Based on these numbers, the probable uncertainty for G is estimated at about +/-7.4 percent.
...
1.2 Principle. The amount of uncaptured VOC emissions (F) from the TTE is calculated as the sum of the products of the VOC content (C), the flow rate (Q) from each uncaptured emissions point, and the sampling time (Θ).
