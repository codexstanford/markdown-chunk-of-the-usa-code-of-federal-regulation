...
(ii) For each day, provide the total SOemitted that day by each emission unit. For any hours on any unit where data for hourly pounds or heat input is missing, identify the unit number and monitoring device that did not produce valid data that caused the missing hour.
