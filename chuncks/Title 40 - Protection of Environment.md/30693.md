...
(1)To demonstrate that a control device meets the required control efficiency, a design evaluation must address the composition and organic HAP concentration of the vent stream entering the control device. A design evaluation also must address other vent stream characteristics and control device operating parameters as specified in any one of paragraphs (a)(1) (i) through (vi) of this section, depending on the type of control device that is used. If the vent stream is not the only inlet to the control device, the efficiency demonstration also must consider all other vapors, gases, and liquids, other than fuels, received by the control device.
