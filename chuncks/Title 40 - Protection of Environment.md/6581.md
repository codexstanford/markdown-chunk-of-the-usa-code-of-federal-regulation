...
The measurement uncertainties are estimated for each captured or uncaptured emissions point as follows: Q=+/-5.5 percent and C=+/-5.0 percent. Based on these numbers, the probable uncertainty for G is estimated at about +/-7.4 percent.
...
1.2 Principle. The amount of VOC captured (G) is calculated as the sum of the products of the VOC content (C), the flow rate (Q), and the sampling time (Θ) from each captured emissions point.
