...
###### Collocated Sampling for Pb.

3.4.3A PSD PQAO must have at least one collocated monitor for each PSD monitoring network.
...
(d) In determining the number of collocated sites required for Pb-PM, PSD monitoring networks for PMshould be treated independently from networks for Pb-PM, even though the separate networks may share one or more common samplers. However, a single quality control monitor that meets the collocation requirements for Pb-PMand PMmay serve as a collocated quality control monitor for both networks. Extreme care must be taken if using a using the filter from a quality control monitor for both PMand Pb analysis. The PMfilter weighing should occur prior to any Pb analysis.
