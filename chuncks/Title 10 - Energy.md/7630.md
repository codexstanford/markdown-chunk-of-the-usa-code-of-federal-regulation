...
(A) When certifying compliance with an EER standard: the energy efficiency ratio (EER in British thermal units per Watt-hour (Btu/Wh)), the coefficient of performance (COP), the rated cooling capacity in British thermal units per hour (Btu/h), and the type(s) of heating used by the basic model (electric, gas, hydronic, none).
